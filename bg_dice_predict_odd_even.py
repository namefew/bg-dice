...
class DiceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dot = int(self.images[original_idx].split('_')[0])   # 标签从0开始
        label = 0 if dot in [1,3,5] else 1
        if self.transform:
...
class CNN():
    ...
    # 训练模型
    def _train_model(self, model, criterion, optimizer, scheduler, num_epochs=50, folder_path='train/new-images',val_folder='train/new-images_val'):
        # 加载数据集
    ...
    def predict_image_top(self, frame: np.ndarray,  background=None):
        ...
        return  [1,3,5,2,4,6] if cls==0 else [6,4,2,5,3,1] ,[cof,0,0,0,0,1-cof]
    ...
    def test(self,image_dir = 'train/new-images1'):
