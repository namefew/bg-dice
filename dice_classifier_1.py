...
class TransitionModel(nn.Module):
    def __init__(self, input_dim=5):  # 修改输入维度为4
        super().__init__()
...
class DiceClassifier:
    def __init__(self, input_size=5,  # 修改输入维度为4
                 hidden_sizes=[12, 128],
                 num_classes=6,
                 batch_size=32,
                 num_epochs=100,
    ...
    def train(self,folder_path ):
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_loss < best_val_loss and (epoch+1)%10 == 0:
                ...
                self.save_model(self.weight_path)
                logging.info(f'New best model saved with val loss: {best_val_loss:.4f}')
            # 早停判断
            self.scheduler.step()  # 更新学习率
        self.save_model(f'{self.weight_path.replace(".pth","_last.pth")}')
        # 新增可视化
